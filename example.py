...
def hovering_example():
    ...
    for _ in range(1000):
        ...
        # Check if PoseSensor data is available and print it
        if "PoseSensor" in state:
            pose = state["PoseSensor"]
            print("PoseSensor data accessed:", pose)

        # Check if RGBCamera data is available and print a confirmation message
        if "RGBCamera" in state:
            rgb_image = state["RGBCamera"]
            print("RGBCamera data accessed. Image shape:", rgb_image.shape)

        # Check if SonarSensor data is available and print a confirmation message
        if "SonarSensor" in state:
            sonar_data = state["SonarSensor"]
            print("SonarSensor data accessed:", sonar_data)
...
def hovering_test():
    ...
    # This command tells the AUV go forward with a power of "10"
    # The last four elements correspond to the horizontal thrusters (see docs for more info)
    command = np.array([0, 0, 0, 10, 10, 10])
    for _ in range(1000):
        state = env.step(command)
        # Check if PoseSensor data is available and print it
        if "PoseSensor" in state:
    ...
    # This command tells the AUV to go down with a power of "10"
    # The first four elements correspond to the vertical thrusters
    command = np.array([-10, -10, -10, 0, 0, 0])
    for _ in range(1000):
        # We alternatively use the act function
        env.act("auv0", command)
        state = env.tick()
